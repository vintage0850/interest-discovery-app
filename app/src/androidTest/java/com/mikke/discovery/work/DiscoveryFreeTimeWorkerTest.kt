...
import com.mikke.discovery.data.calendar.CalendarEventListResponse
import com.mikke.discovery.data.calendar.CalendarResult
import com.mikke.discovery.data.calendar.GoogleAuthManager
import com.mikke.discovery.data.calendar.GoogleCalendarApi
import com.mikke.discovery.data.calendar.GoogleCalendarSync
import com.mikke.discovery.shared.discovery.DiscoveryRepository
import com.mikke.discovery.shared.discovery.Experiment
import com.mikke.discovery.shared.discovery.FakeDiscoveryRepository
...
class DiscoveryFreeTimeWorkerTest {
    ...
    fun 空き時間が無ければスキップする() = runBlocking {
        ...
        val worker = buildWorker(
            ...
            calendarSync = FakeCalendarSync(
                events = listOf(
                    com.mikke.discovery.data.calendar.CalendarEventSlot(now, windowEnd)
                )
            ),
            repository = FakeDiscoveryRepository(enableArtificialDelay = false),
    ...
    private class FakeCalendarSync(
        private val events: List<com.mikke.discovery.data.calendar.CalendarEventSlot>
    ) : GoogleCalendarSync(GoogleAuthManager.get(dummyContext()), DummyApi, NoOpLogger) {
        override suspend fun listTodayEvents(
            now: Instant,
            zoneId: ZoneId
        ): CalendarResult<List<com.mikke.discovery.data.calendar.CalendarEventSlot>> =
    ...
    private object DummyApi : GoogleCalendarApi {
        override suspend fun insertEvent(
            ...
            event: com.mikke.discovery.data.calendar.CalendarEventRequest
    ...
    private object NoOpLogger : com.mikke.discovery.data.calendar.CalendarLogger {
